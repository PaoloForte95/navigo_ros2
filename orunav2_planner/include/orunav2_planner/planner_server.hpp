...
#ifndef ORUNAV2_PLANNER__PLANNER_SERVER_HPP_
#define ORUNAV2_PLANNER__PLANNER_SERVER_HPP_
 ...
 * interface and hosts various plugins of different algorithms to compute plans.
 */
class EclPlannerServer : public nav2_util::LifecycleNode
{
public:
  /**
   ...
   * @param options Additional options to control creation of the node.
   */
  explicit EclPlannerServer(const rclcpp::NodeOptions & options = rclcpp::NodeOptions());
  /**
   * @brief A destructor for orunav2_planner::EclPlannerServer
   */
  ~EclPlannerServer();

  using PlannerMap = std::unordered_map<std::string, orunav2_core::GlobalEclPlanner::Ptr>;
...
protected:
  ...
  // Planner
  PlannerMap planners_;
  pluginlib::ClassLoader<orunav2_core::GlobalEclPlanner> gp_loader_;
  std::vector<std::string> default_ids_;
  std::vector<std::string> default_types_;
  std::vector<std::string> planner_ids_;
  std::vector<std::string> planner_types_;
...
#endif  // ORUNAV2_PLANNER__PLANNER_SERVER_HPP_
