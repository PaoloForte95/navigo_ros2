...
: rclcpp_lifecycle::LifecycleNode(node_name, ns, options)
{
  ...
  weather_condition_pub_ = create_publisher<navigo2_msgs::msg::WeatherState>("/weather_condition", 1);
...

{
    ...
    std::shared_ptr<navigo2_selector::Image> p = std::make_shared<navigo2_selector::Image>(node, source_type, tf_buffer, base_frame_id, odom_frame_id, transform_tolerance, source_timeout);
    p->configure();
    source_.push_back(p);

    client_node_ = rclcpp::Node::make_shared("client_node");
    
    weather_condition_client_ = client_node_->create_client<navigo2_msgs::srv::GetWeatherCondition>(service_name,  rclcpp::ServicesQoS().get_rmw_qos_profile());
...
void WeatherDetector::process()
{
  ...
  auto request = std::make_shared<navigo2_msgs::srv::GetWeatherCondition::Request>();
  request->image = data_;
  ...
  std::unique_ptr<navigo2_msgs::msg::WeatherState> weather_state_msg =std::make_unique<navigo2_msgs::msg::WeatherState>();
