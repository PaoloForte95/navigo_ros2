...
namespace orunav2_generic {
  ...
  class SubsamplePath {
  public:
  SubsamplePath(const orunav2_generic::PathInterface &p, double minDist, double maxDist, double maxRotation) : orig_path(p) {
      this->extractPath(minDist, maxDist, maxRotation);
    }
    const orunav2_generic::Path& getOrigPath() const { return orig_path; }
    const orunav2_generic::Path& getPath() const { return path; }
  ...
  private:
      ...
      for (size_t i = 0; i < orig_path.sizePath(); i++)
	{
	  ...
	  if (active && orunav2_generic::getDistBetween(orig_path.getPose2d(i), orig_path.getPose2d(it)) <= minDist)
	    {
	      subsampled_idx.push_back(i);
	      path.addPathPoint(orig_path.getPose2d(i), orig_path.getSteeringAngle(i));
	      active = false;
	  ...
	  if (orunav2_generic::addStepPose2d(path.poses.back(), orig_path.getPose2d(i), maxDist, maxRotation))
	    {
	      subsampled_idx.push_back(i);
	      path.addPathPoint(orig_path.getPose2d(i), orig_path.getSteeringAngle(i));
	      continue;
    ...
    orunav2_generic::Path orig_path;
