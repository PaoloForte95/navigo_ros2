...
#include "navthon_behavior_tree/plugins/decorator/human_detection_controller.hpp"
...
rclcpp::QoS scan_qos = rclcpp::SensorDataQoS();  // set to default
  detection_msg_ = navthon_msgs::msg::HumanDetection();
  detection_msg_.human_detected = false;
  getInput("topic", topic_);
  getInput("waiting_time", waiting_time_);
  node_ = config().blackboard->get<rclcpp::Node::SharedPtr>("node");

  data_sub_ = node_->create_subscription<navthon_msgs::msg::HumanDetection>(topic_, scan_qos,std::bind(&HumanDetectionController::dataCallback, this, std::placeholders::_1));
...
void HumanDetectionController::dataCallback(navthon_msgs::msg::HumanDetection msg)
{
  detection_msg_ = msg;
...
{
  factory.registerNodeType<navthon::behavior_tree::HumanDetectionController>("HumanDetectionController");
}
