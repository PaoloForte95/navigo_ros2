...
#include "navigo2_selector/selectors/costmap_selector.hpp"
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  for (std::string source_name : source_names) {
    ...
    if (source_type == "scan") {
      std::shared_ptr<navigo2_selector::Scan> s = std::make_shared<navigo2_selector::Scan>(node, name + "." + source_name, costmap_ros->getTfBuffer(), _base_frame_id, _odom_frame_id, transform_tolerance, source_timeout);
      s->configure();
      _sources.push_back(s);

    } else if (source_type == "image") {
      std::shared_ptr<navigo2_selector::Image> p = std::make_shared<navigo2_selector::Image>(node, name + "." + source_name, costmap_ros->getTfBuffer(), _base_frame_id, _odom_frame_id, transform_tolerance, source_timeout);
      p->configure();
      _sources.push_back(p);
    }else if (source_type == "pointcloud") {
      std::shared_ptr<navigo2_selector::PointCloud> p = std::make_shared<navigo2_selector::PointCloud>(node, name + "." + source_name, costmap_ros->getTfBuffer(), _base_frame_id, _odom_frame_id, transform_tolerance, source_timeout);
      p->configure();
      _sources.push_back(p);
  ...
  const std::vector<std::string> & planner_ids)
{
  ...
  for (std::shared_ptr<navigo2_selector::Source> source : _sources) {
...
PLUGINLIB_EXPORT_CLASS(navigo2_selector::CostmapSelector, navigo2_core::GlobalPlannerSelector)
