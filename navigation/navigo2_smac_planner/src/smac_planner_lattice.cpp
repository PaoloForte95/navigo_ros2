...
LatticePlanner::~LatticePlanner()
{
  RCLCPP_INFO( logger_, "Destroying LatticePlanner", name_.c_str());
}
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  RCLCPP_INFO(logger_, "Configuring LatticePlanner...");
  ...
  nav2_util::declare_parameter_if_not_declared(
    node, name + ".primitives_directory", rclcpp::ParameterValue(""));   
  node->get_parameter(name + ".primitives_directory", primitives_directory);

  nav2_util::declare_parameter_if_not_declared(
    node, name + ".primitives", rclcpp::ParameterValue(""));   
  node->get_parameter(name + ".primitives", primitives);
  LatticeMetadata::PRIMITIVES_DIR = primitives_directory;
  ...
  planner_ = new SmacLatticePlanner(primitives,planner_params,search_params);
  ...
  RCLCPP_INFO( logger_, "Configured LatticePlanner! ");
}

void LatticePlanner::activate()
{
  RCLCPP_INFO(
    logger_, "Activating plugin %s of type LatticePlanner",
    name_.c_str());
  raw_plan_publisher_->on_activate();
  auto node = node_.lock();
  // Add callback for dynamic parameters
  dyn_params_handler_ = node->add_on_set_parameters_callback(
    std::bind(&LatticePlanner::dynamicParametersCallback, this, std::placeholders::_1));
}

void LatticePlanner::deactivate()
{
  RCLCPP_INFO(
    logger_, "Deactivating plugin %s of type LatticePlanner",
    name_.c_str());
...
{
  RCLCPP_INFO(
    logger_, "Cleaning up plugin %s of type LatticePlanner",
    name_.c_str());
  ...
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  planner_->setCostMap(map.get());


  if (!getArticulatedJointAngle(steering_start)) {
        RCLCPP_WARN(logger_, "failed to query the articulated joint angle, check front_frame_id and rear_frame_id. Set start steering angle to 0");
  }

  RCLCPP_INFO(logger_, "start : [%f,%f,%f](%f)", start.pose.position.x, start.pose.position.y,tf2::getYaw(start.pose.orientation), steering_start);
  RCLCPP_INFO(logger_, "goal :  [%f,%f,%f](%f)", goal.pose.position.x, goal.pose.position.y,tf2::getYaw(goal.pose.orientation), steering_goal);
  ...
  bool LatticePlanner::getArticulatedJointAngle(double &angle) const
  {
...
LatticePlanner::dynamicParametersCallback(std::vector<rclcpp::Parameter> parameters)
{
...
PLUGINLIB_EXPORT_CLASS(navigo2_smac_planner::LatticePlanner, nav2_core::GlobalPlanner)
