...
#include "navthon_smoother/nav2_smoother.hpp"
...
: LifecycleNode("smoother_server", "", options),
  lp_loader_("navthon_core", "navthon_core::Smoother"),
  default_ids_{"simple_smoother"},
  default_types_{"navthon_smoother::SimpleSmoother"}
{
...
bool SmootherServer::loadSmootherPlugins()
{
  ...
  for (size_t i = 0; i != smoother_ids_.size(); i++) {
    try {
      ...
      navthon_core::Smoother::Ptr smoother =
...
RCLCPP_COMPONENTS_REGISTER_NODE(navthon_smoother::SmootherServer)
