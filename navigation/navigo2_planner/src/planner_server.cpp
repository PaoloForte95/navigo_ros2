...
#include "navigo2_planner/planner_server.hpp"

using namespace std::chrono_literals;
using rcl_interfaces::msg::ParameterType;
using std::placeholders::_1;

namespace navigo2_planner
{
...
PlannerServer::computePlan()
{
  ...
  try {
    ...
    auto start_time_ct = std::chrono::high_resolution_clock::now();
    RCLCPP_INFO(get_logger(), " Computing the plan with planning algorithm %s", goal->planner_id.c_str());
    result->path = getPlan(start, goal_pose, goal->planner_id);

    auto end_time_ct = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> duration = end_time_ct - start_time_ct;
    RCLCPP_INFO(get_logger(), "Global Path Planning Computational Time: %f ", duration.count());
    std::ofstream outFile("computationalTime.txt", std::ios::app);
      if (outFile.is_open()) {
...
RCLCPP_COMPONENTS_REGISTER_NODE(navigo2_planner::PlannerServer)
