...
#include "navthon_selector/global_planner_selector_server.hpp"
...
: nav2_util::LifecycleNode("selector_server", "", options),
  gp_loader_("navthon_core", "navthon_core::GlobalPlannerSelector"),
  default_ids_{"CostmapBased"},
  default_types_{"navthon_selector::CostmapSelector"}
{
...
PlannerSelectorServer::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  for (size_t i = 0; i != selector_ids_.size(); i++) {
    try {
      ...
      navthon_core::GlobalPlannerSelector::Ptr planner_selector =
...
PlannerSelectorServer::on_activate(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  is_planner_valid_service_ = node->create_service<navthon_msgs::srv::IsPlannerValid>(
    "is_path_valid",
    std::bind(
...
RCLCPP_COMPONENTS_REGISTER_NODE(navthon_selector::PlannerSelectorServer)
