...
SmacPlannerAStar::SmacPlannerAStar()
: _a_star(nullptr),
...
{
  RCLCPP_INFO(
    _logger, "Destroying plugin %s of type SmacPlannerAStar",
    _name.c_str());
}

void SmacPlannerAStar::configure(
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  RCLCPP_INFO(_logger, "Configuring %s of type SmacPlannerAStar", name.c_str());
  ...
  _motion_model = nav2_smac_planner::MotionModel::TWOD;
  ...
  _collision_checker = nav2_smac_planner::GridCollisionChecker(_costmap, 1 /*for 2D, most be 1*/, node);
  ...
  _a_star = std::make_unique<nav2_smac_planner::AStarAlgorithm<nav2_smac_planner::Node2D>>(_motion_model, _search_info);
  _a_star->initialize(
    _allow_unknown,
    _max_iterations,
  ...
  RCLCPP_INFO(
    _logger, "Configured plugin %s of type SmacPlannerAStar with "
...
void SmacPlannerAStar::activate()
{
  RCLCPP_INFO(
    _logger, "Activating plugin %s of type SmacPlannerAStar",
    _name.c_str());
  ...
  _dyn_params_handler = node->add_on_set_parameters_callback(
    std::bind(&SmacPlannerAStar::dynamicParametersCallback, this, _1));
}

void SmacPlannerAStar::deactivate()
{
  RCLCPP_INFO(
    _logger, "Deactivating plugin %s of type SmacPlannerAStar",
    _name.c_str());
...
void SmacPlannerAStar::cleanup()
{
  RCLCPP_INFO(
    _logger, "Cleaning up plugin %s of type SmacPlannerAStar",
    _name.c_str());
  _a_star.reset();
  _smoother.reset();
  if (_costmap_downsampler) {
...
SmacPlannerAStar::dynamicParametersCallback(std::vector<rclcpp::Parameter> parameters)
{
...
PLUGINLIB_EXPORT_CLASS(navthon_smac_planner::SmacPlannerAStar, nav2_core::GlobalPlanner)
