...
#include <navthon_util/conversion_utils.hpp>
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  nav2_util::declare_parameter_if_not_declared(
    node, name + ".motion_primitives_directory", rclcpp::ParameterValue(
      ament_index_cpp::get_package_share_directory("navthon_smac_planner") + 
  ...
  //_collision_checker = navthon_smac_planner::CollisionDetector(_costmap);
  ...
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  _path_finder->addMission(&vm);
   ...
   if (solution_found) {
      ...
      navthon_generic::Path path_min_dist = navthon_generic::minIncrementalDistancePath(path, _min_incr_path_dist);
      // Second requirment (path states are not allowed to change direction of motion without any intermediate points).
      navthon_generic::Path path_dir_change = navthon_generic::minIntermediateDirPathPoints(path_min_dist);
...
PLUGINLIB_EXPORT_CLASS(navthon_smac_planner::SmacPlannerLattice, nav2_core::GlobalPlanner)
