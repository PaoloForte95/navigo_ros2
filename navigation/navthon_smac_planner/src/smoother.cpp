...
Smoother::Smoother(const nav2_smac_planner::SmootherParams & params)
{
  ...
  const double & max_time)
{
  ...
  while (change >= tolerance_) {
    ...
    for (unsigned int i = 1; i != path_size - 1; i++) {
      ...
      if (cost > nav2_smac_planner::MAX_NON_OBSTACLE && cost != nav2_smac_planner::UNKNOWN) {
        RCLCPP_DEBUG(
          rclcpp::get_logger("SmacPlannerSmoother"),
  ...
  const nav2_costmap_2d::Costmap2D * costmap)
{
  ...
  for (double i = 0; i <= expansion.path_end_idx; i++) {
    ...
    if (static_cast<float>(costmap->getCost(mx, my)) >= nav2_smac_planner::INSCRIBED) {
