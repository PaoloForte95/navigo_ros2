...
#include "navthon_smac_planner/disc_world_visualizer.hpp"
...
void DiscWorldVisualizer::drawLHDConfiguration(LHDConfiguration* conf, int R, int G, int B) {
	...
	navthon_smac_planner::LHDModel* model = dynamic_cast<navthon_smac_planner::LHDModel*> (conf->getMission()->getVehicleModel());
	double scale = scaleFactor_ / WP::WORLD_SPACE_GRANULARITY;
