...
namespace navthon_generic
{
  ...
  inline std::vector<navthon_generic::Path> loadPathDirTextFile(const std::string &dirName) {
	...
	navthon_generic::Path p = navthon_generic::loadPathTextFile(file_names[i]);
	if (p.sizePath() == 0)
	  {
  ...
  inline void saveQuaternionPathTextFile(const PathInterface &path, const std::string &fileName) { 
    ...
    for (unsigned int i = 0; i < path.sizePath(); i++)
      {
	Eigen::Quaterniond q = navthon_generic::getQuaterion(path.getPose2d(i));
	ofs << path.getPose2d(i)(0) << " "
	    << path.getPose2d(i)(1) << " "
	    << q.z() << " "
	    << q.w() << " "
  ...
  inline Trajectory loadTrajectoryTextFile(const std::string &fileName) {
    ...
    navthon_generic::Trajectory traj;
    std::ifstream ifs;
    ifs.open(fileName.c_str());
    if (!ifs.is_open()) {
      std::cerr << __FILE__ << ":" << __LINE__ << " cannot open file : " << fileName << std::endl;
  ...
  inline void saveTraversedDistanceTrajectoryTextFile(const TrajectoryInterface &traj, const std::string &fileName) {
	...
	dist_sum += navthon_generic::getDistBetween(traj.getPose2d(i), traj.getPose2d(i+1));
      }
    }	  
  ...
  inline CoordinatedTimes loadCoordinatedTimesTextFile(const std::string &fileName) {
    std::vector<double> tmp = navthon_generic::loadDoubleVecTextFile(fileName);
