...
def main():
    ...
    parser.add_argument('-roll', type=float, default=0,
                        help='the roll component of the initial position [radians]')
    parser.add_argument('-pitch', type=float, default=0,
                        help='the pitch component of the initial position [radians]')
    ...
    node.get_logger().info('spawning `{}` on namespace `{}` at {}, {}, {}, {}, {}, {}'.format(
        args.robot_name, args.robot_namespace, args.x, args.y, args.z, args.roll, args.pitch, args.yaw ))
    ...
    for plugin in root.iter('plugin'):
        ...
        if 'turtlebot3_diff_drive' in plugin.attrib.values() or 'steer_drive_controller' in plugin.attrib.values() :
    ...
    quat = get_quaternion_from_euler(args.roll, args.pitch, args.yaw)
