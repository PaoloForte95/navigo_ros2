...
#include "orunav2_smoother/nav2_smoother.hpp"
...
: LifecycleNode("smoother_server", "", options),
  lp_loader_("orunav2_core", "orunav2_core::Smoother"),
  default_ids_{"simple_smoother"},
  default_types_{"orunav2_smoother::SimpleSmoother"}
{
...
bool SmootherServer::loadSmootherPlugins()
{
  ...
  for (size_t i = 0; i != smoother_ids_.size(); i++) {
    try {
      ...
      orunav2_core::Smoother::Ptr smoother =
...
RCLCPP_COMPONENTS_REGISTER_NODE(orunav2_smoother::SmootherServer)
