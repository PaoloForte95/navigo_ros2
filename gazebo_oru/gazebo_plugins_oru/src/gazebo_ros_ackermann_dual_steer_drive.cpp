...
{
    parent = _parent;
    wheel_diameter_ = _sdf->Get<double>("wheelDiameter", 0.59).first;
    steer_torque_ =  _sdf->Get<double>("steerTorque", 5.0).first;
    drive_torque_ =  _sdf->Get<double>("driveTorque", 5.0).first;

    std::string command_topic_ = _sdf->Get<std::string>("commandTopic", "cmd_vel" ).first;
    std::string odometry_topic_ = _sdf->Get<std::string>("odometryTopic", "odom" ).first;
    std::string odometry_frame_ = _sdf->Get<std::string>("odometryFrame", "odom" ).first;
    std::string robot_base_frame_ = _sdf->Get<std::string>("robotBaseFrame", "base_link" ).first;

    update_rate_ =  _sdf->Get<double>("updateRate", 100.0).first;
    wheel_acceleration_ =  _sdf->Get<double>("wheelAcceleration", 0).first;
    wheel_deceleration_ =  _sdf->Get<double>("wheelDeceleration", wheel_acceleration_).first;

    { 
      double x, y;
      x =  _sdf->Get<double>("fl_wheel_pos_x_", 3.2).first;
      y =  _sdf->Get<double>("fl_wheel_pos_y_", 1.11).first;
      ackermann_model_.addWheel(orunav_generic::SteerDriveWheel(x,y));
      x =  _sdf->Get<double>("fl_wheel_pos_x_", 3.2).first;
      y =  _sdf->Get<double>("fl_wheel_pos_y_", -1.11).first;
      ackermann_model_.addWheel(orunav_generic::SteerDriveWheel(x,y));
      x =  _sdf->Get<double>("fl_wheel_pos_x_", 0.0).first;
      y =  _sdf->Get<double>("fl_wheel_pos_y_", 1.11).first;
      ackermann_model_.addWheel(orunav_generic::SteerDriveWheel(x,y));
      x =  _sdf->Get<double>("fl_wheel_pos_x_", 0.).first;
      y =  _sdf->Get<double>("fl_wheel_pos_y_", -1.11).first;
      ackermann_model_.addWheel(orunav_generic::SteerDriveWheel(x,y));
      x =  _sdf->Get<double>("fl_wheel_pos_x_", 3.2).first;
      y =  _sdf->Get<double>("fl_wheel_pos_y_", 0.).first;
      ackermann_model_.setSteerDriveWheelPos(x, y);
    }
    publishWheelTF_ = _sdf->Get<bool>("publishWheelTF", false).first;
    publishWheelJointState_ = _sdf->Get<bool>("publishWheelJointState", false).first;

    // Virual steering wheel
    joint_name = _sdf->Get<std::string>("frontSteerJoint", "front_steer_joint").first;
    joint_front_steer_ = parent->GetJoint(joint_name);
    joint_name = _sdf->Get<std::string>("rearSteerJoint", "rear_steer_joint").first;
    joint_rear_steer_ = parent->GetJoint(joint_name);
    
    // 4WD steering
    joint_name = _sdf->Get<std::string>("wheelFrontLeftSteerJoint", "base2steering_fl_joint").first;
    joint_wheel_fl_steer_ = parent->GetJoint(joint_name);
    joint_name = _sdf->Get<std::string>("wheelFrontRightSteerJoint", "base2steering_fr_joint").first;
    joint_wheel_fr_steer_ = parent->GetJoint(joint_name);
    joint_name = _sdf->Get<std::string>("wheelBackLeftSteerJoint", "base2steering_bl_joint").first;
    joint_wheel_bl_steer_ = parent->GetJoint(joint_name);
    joint_name = _sdf->Get<std::string>("wheelBackRightSteerJoint", "base2steering_br_joint").first;
    joint_wheel_br_steer_ = parent->GetJoint(joint_name);


    // 4WD
    joint_name = _sdf->Get<std::string>("wheelFrontLeftDriveJoint", "steering2wheel_fl_joint").first;
    joint_wheel_fl_drive_ = parent->GetJoint(joint_name);
    joint_name = _sdf->Get<std::string>("wheelFrontRightDriveJoint", "steering2wheel_fr_joint").first;
    joint_wheel_fr_drive_ = parent->GetJoint(joint_name);
    joint_name = _sdf->Get<std::string>("wheelBackLeftDriveJoint", "steering2wheel_bl_joint").first;
    joint_wheel_bl_drive_ = parent->GetJoint(joint_name);
    joint_name = _sdf->Get<std::string>("wheelBackRightDriveJoint", "steering2wheel_br_joint").first;
    joint_wheel_br_drive_ = parent->GetJoint(joint_name);
    ...
    if ( this->publishWheelJointState_ ) {
        joint_state_publisher_ = gazebo_ros_.Get()->create_publisher<sensor_msgs::msg::JointState> ( "joint_states", 1000 );
        ROS_INFO ( "%s: Advertise joint_states!", gazebo_ros_.Get()->get_name () );
    }
    ...
    ROS_INFO ( "%s: Try to subscribe to %s!", gazebo_ros_.Get()->get_name (), command_topic_.c_str() );

    ros::SubscribeOptions so =
        ros::SubscribeOptions::create<geometry_msgs::msg::Twist> ( command_topic_, 1,
                boost::bind ( &GazeboRosAckermannDualSteerDrive::cmdVelCallback, this, _1 ),
                ros::VoidPtr(), &queue_ );

    cmd_vel_subscriber_ = gazebo_ros_->node()->subscribe ( so );
    ROS_INFO ( "%s: Subscribe to %s!", gazebo_ros_.Get()->get_name (), command_topic_.c_str() );

    odometry_publisher_ = gazebo_ros_->node()->advertise<nav_msgs::Odometry> ( odometry_topic_, 1 );
    ROS_INFO ( "%s: Advertise odom on %s !", gazebo_ros_.Get()->get_name (), odometry_topic_.c_str() );
